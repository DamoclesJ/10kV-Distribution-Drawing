...
- **WP-EM-07A State:** Implementation Complete / Code Review Passed / Windows Automated Tests Passed / Windows Professional Acceptance Fix-2 In Progress / Closure Blocked
- **Post-EM-07 Sequencing Amendment:** Completed / Frozen
- **Current Work Package:** WP-EM-07A Transformer & Pole-Device Grounding Amendment — Windows Professional Acceptance Fix-2
...
- **Blockers:** WP-EM-07A Windows professional acceptance requires Fix-2 closure for dual short-OHL GAP placement, Transformer-centric workflow, manual Transformer leader direction, and 90° route continuity. WP-EM-07B remains Not Started; generic drag and routing stabilization remains assigned to WP-EM-08.
...
The Post-EM-07 Sequencing Amendment inserts two independent Post-EM-07 / Pre-EM-08 Work Packages without reopening WP-EM-06 or WP-EM-07. WP-EM-07A implementation and automated review gates have completed, but Windows professional acceptance remains blocked pending Fix-2. The Windows Professional Acceptance Amendment adds `GroundingAccessPlacementSide` (`PoleSide | AdjacentEndpointSide`) and `GroundingAccessLineSide.TransformerSide`, allowing two independent conductor GAP identities on the same legal pole-mounted Transformer short OHL. V7 remains additive-compatible: old records without placement normalize to `PoleSide`; no V8 migration is authorized. WP-EM-07B, WP-EM-08, and WP-EM-09 remain Planned / Not Started.
