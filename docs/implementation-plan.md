...
第一阶段明确不实现 CAD DWG 兼容、自动计算潮流、电气仿真、数据库、云同步和多用户。PT 柜、DTU 柜已有详细模型设计，但未在本次 MVP 清单中作为独立必验设备；除非后续更新需求基线，否则不影响 MVP 完成判定。
...
| 层次 | 对象作用 | 示例 |
| --- | --- | --- |
| 领域对象 | 保存业务语义和不变量 | `Equipment`、`Terminal`、`Connection`、`RingCabinetAssembly` |
...
| 设备 | `Equipment` | 设备类型、标识、名称、电压、状态和端子 |
| 开关设备 | `SwitchEquipment` 或 Equipment + SwitchState 组合 | 独立拉开/合入等操作状态和内部导通定义 |
...
| 水泥杆 | `PoleEquipment` | 保存杆号、位置和可挂接柱上设备的结构引用 |
| 柱上设备 | `PoleMountedEquipment` | 保存所属杆位、设备类型、端子及适用的开关状态 |
| 连接路线 | `ConnectionRoute` | 保存连接线的人工折点和路由模式 |
| 工作接地线 | `GroundingMeasure` | 非当前 MVP；范围确认后再实现装设端子、编号和确认状态 |
| 工作范围 | `WorkScope` | 非当前 MVP；范围确认后再实现参与设备、连接、边界和说明 |
| 标注 | `Annotation` | 文字、引线和非导电说明 |
| 环网柜组合 | `RingCabinetAssembly` | 柜型、普通间隔、主母线和结构约束 |
| 普通间隔 | `RingCabinetBay` | 设备组合、主母线引用、对外回路端子和间隔顺序 |
| PT 单元 | `PtUnit` | 非当前 MVP 独立验收项；保留左右位置、表现形态和主母线端子设计 |
| DTU 柜 | `DtuCabinet` | 非当前 MVP 独立验收项；保留无电气端子和操作状态的柜体设计 |
| 布局 | `DiagramLayout` | 页面、元素位置、旋转、层级、标签锚点和路线 |

### 5.3 继承与组合

第一版采用“浅层继承 + 组合”方式：

- `Equipment` 提供所有设备共有的标识、类型、名称、电压和端子集合。
- 开关操作位置使用独立的状态值对象或能力接口，不为“断路器拉开”“断路器合入”建立两个类。
- 设备内部导通关系由设备定义和当前操作状态共同产生。
- 环网柜、间隔、PT 和 DTU 使用明确组合对象，因为它们具有结构不变量。
...
- 电压等级、设备类型、操作状态、电气状态、工作角色和生命周期状态分别建模，不使用一个通用字符串状态。
- 页面和布局坐标以毫米为持久化单位；渲染时转换为 WPF DIP，换算关系为 1 英寸 = 25.4 毫米 = 96 DIP。
- 旋转角度、元素尺寸、连接折点均保存为与屏幕 DPI 无关的逻辑值。

### 5.5 环网柜对象映射

普通负荷开关型实例化时：

- 根据 3/4/5/6 间隔数量创建对应 `RingCabinetBay`。
- 每个间隔创建一台负荷开关和一台接地刀闸。
- 每台开关独立持有操作状态。
...
PT、DTU 按现有设计保留如下映射，但不作为当前 MVP 独立验收项：

- PT 最多一个，保存特殊间隔/独立柜体形态和左/右位置，电气端子连接主母线，无开关操作状态。
- DTU 最多一个，只保存左/右位置、柜体名称和布局，无端子、无开关状态、无电气状态计算。
...
三工位设备、运行/检修位置、生命周期图元以及 PT、DTU 的表现设计保留在专业设计文档中，但未进入本次 MVP 时不实现对应状态选择器。
...
- 环网柜组合及普通间隔。
- 当前 MVP 设备所需的文字标注。
- 元素位置、旋转、层级、标签锚点和连接折点。
- 绘图规则和图元库版本。

V1 文件 DTO 只包含实际进入 MVP 的模型字段。PT、DTU、工作接地线和工作范围虽然已有设计映射，但在其未进入需求基线前，不为未来能力预设空字段。
...
退出条件：目标电脑可运行，JPG 清晰，目标打印机不裁切，红色/黑色或蓝色可辨认。若失败，先调整路线，不进入正式画布开发。

### M1：解决方案骨架与领域模型（约 1～2 周）

工作内容：

- 建立项目依赖边界。
- 实现文档、设备、端子、电气节点、连接、状态和布局值对象。
- 实现普通环网柜、一二次融合环网柜、电缆、水泥杆、架空线路、柱上设备的模型映射及结构不变量。
- 建立应用命令和撤销/重做合同。

退出条件：不依赖 WPF 即可构造和校验 3/4/5/6 间隔普通柜、4/6 间隔一二次融合柜及“环网柜—电缆—水泥杆—架空线路—柱上设备”拓扑，并验证适用设备状态独立。

### M2：图元库和离屏渲染内核（约 1～2 周）

工作内容：

- 将普通环网柜、一二次融合环网柜、电缆、水泥杆、架空线路及柱上设备的权威图元整理为 WPF 矢量定义。
...
- 实现水泥杆的杆号、位置属性和柱上设备挂接关系。
- 实现柱上断路器、柱上负荷开关、柱上隔离开关、跌落式熔断器的适用属性和状态编辑。

退出条件：两类环网柜的必需间隔和独立状态通过验证，六类 MVP 设备均可拖放、移动、编辑属性，并在适用时修改状态。
...
退出条件：包含全部六类 MVP 设备的图纸保存后重新打开，领域语义、连接、状态和布局完全一致；中断保存不破坏原文件。
...
- 使用普通环网柜场景和一二次融合环网柜场景两类脱敏案例；每类均包含电缆、水泥杆、架空线路和柱上设备。
...
- 领域测试：六类 MVP 设备构造、设备状态独立、端子连接、环网柜数量和结构不变量。
- 命令测试：执行、撤销、重做后模型一致。
...
这些排除项不得成为 MVP 里程碑的隐含前置条件，也不为其建立未使用的业务模块。状态由用户手工设置，工程数据保存在本地工程文件中。
