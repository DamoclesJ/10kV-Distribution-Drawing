...
- `README.md` 为空文件。
- `docs/requirements.md` 已定义第一阶段 MVP 范围；`docs/drawing-rule.md` 和 `docs/ring-cabinet-design.md` 已形成专项设计；`docs/equipment-model.md` 仍待补充。
...
4. 保存后可无损重新打开继续编辑。
5. JPG 和打印与画布使用同一绘制语义。
6. 两类端到端金标准场景通过配电专业人员复核。
...
PT、DTU 已有模型设计，但未作为本次 MVP 独立验收对象。其他规范图元和典型场景不因出现在参考资料中而自动进入第一阶段。
...
| 里程碑 | 主要工作 | 可验证产物 |
| --- | --- | --- |
| M0：需求与图元基线（1 周） | 确认目标 Windows、票面尺寸、六类设备图元及两类验收场景 | 已确认的范围、图元清单和金标准图纸 |
| M1：领域与文件内核（1～2 周） | 确定设备—端子—连接、状态、布局和工程文件模型 | 两类环网柜结构及代表性连接可校验；保存往返语义不变 |
...
两类场景均应覆盖拖放、移动、属性编辑、状态修改、保存重开、JPG 和打印。变压器、箱式站等其他规范场景放到 MVP 后续迭代。
...
| 风险 | 影响 | 建议应对 |
| --- | --- | --- |
| 通用设备模型文档仍为空 | 设备字段和端子语义可能在编码期反复修改 | 编码前按六类 MVP 设备补齐 `docs/equipment-model.md` 并由业务方确认 |
...
1. 按已确认的六类设备补齐 `docs/equipment-model.md`，明确字段、端子、连接能力和状态能力。
2. 由配电专业人员确认首批权威图元、两类环网柜模板及两类金标准图纸。
3. 明确第 8 节尚未确定的 Windows、打印、颜色和 JPG 参数，并同步更新 `docs/requirements.md` 与 `docs/implementation-plan.md`。

以上三项完成后，再冻结第一版工程文件 V1 和领域接口，进入 MVP 实现。
