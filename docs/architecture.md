...
- 产品形态：离线优先的桌面应用，核心绘图能力同时保持可嵌入浏览器的可能性。
- 软件结构：单体应用内的分层、模块化架构；第一阶段不拆微服务，不建设协同后端。
- 核心模型：基于“设备—端子—连接”的电气拓扑图模型，将电气拓扑、设备状态、安全措施和画布布局分开保存。
- 绘图方式：使用 SVG 作为第一阶段的主渲染技术，以便获得清晰打印、稳定缩放、可选中图元和可测试输出。
- 规则能力：颜色、线型、图元形态由设备语义和状态派生；规则引擎负责缺项、冲突、明显断开点和接地保护范围等检查。
- 数据文件：采用带版本号的结构化工程文件；图像或 PDF 只作为导出物，不能作为可编辑源数据。
- MVP 目标：完成工作票附图的创建、编辑、校验、保存和打印闭环，优先支持环网柜、配电变压器、箱式站高压柜及电缆搭火/挑火等代表性场景。
...
- `README.md` 为空文件。
- `docs/requirements.md`、`docs/drawing-rule.md`、`docs/equipment-model.md`、`docs/ring-cabinet-design.md` 均为空文件。
- 仓库中尚无代码、技术栈约束或既有数据格式。
- 可确认的业务要求主要来自 reference 目录中的 V7 规范文件。该文件包含各专业绘图要求、配电图元表，以及 12 类配电典型现场附图。

因此，本文把规范文件中的明确表述视为需求基线；对运行平台、系统集成、审批流程等尚未给出的内容，只提出建议，不视为已确认需求。
...
- 从空白图纸或典型模板创建工作票附图。
- 放置配电设备、修改设备名称和状态、连接设备端子。
- 表达带电、停电、工作范围、接地线和接地刀闸。
- 保存、打开、另存、撤销、重做和自动恢复。
- 导出 PDF 和高分辨率图片，并完成 A4/A3 彩色打印预览。
- 执行核心规范检查并输出可定位的问题列表。
...
- **语义模型是唯一事实源**：画布只负责编辑和显示，不能把 SVG 节点或像素坐标当作业务数据。
...
- **安全结论需人工确认**：自动推导用于辅助和发现冲突，不能在没有确认的情况下替代工作票编制人员判断。
...
flowchart TB
    ...
    Symbols["图元与组合模板库<br/>SVG 图元、端子定义、状态变体"]
    Render["渲染与排版<br/>颜色派生、标签布局、分页、图例"]
    Store["工程文件与版本迁移<br/>保存、自动恢复、导入导出"]
    Export["输出适配器<br/>PDF、PNG、打印"]
...
| 模块 | 主要职责 | 不应承担的职责 |
| --- | --- | --- |
| 应用界面 | 命令入口、属性编辑、问题导航、文档管理 | 不直接修改 SVG 或持久化文件 |
...
| 渲染与排版 | 将模型稳定渲染为 SVG，处理标签、颜色、线型、页面边界 | 不作为数据源反向解析业务模型 |
| 工程文件与迁移 | 序列化、文件校验、版本升级、自动恢复 | 不保存仅对本次渲染有效的临时对象 |
| 输出适配器 | PDF、PNG、打印及输出前检查 | 不绕过统一渲染结果另画一份图 |

### 4.3 技术形态建议

在技术栈尚未确定的情况下，建议采用“Web 绘图内核 + 桌面壳”的形态：

- 编辑器与业务模块使用 TypeScript，UI 可选 React。
- 第一阶段使用 SVG 渲染；图元数量达到数千并出现性能问题后，再评估 Canvas/WebGL，而不是提前引入双渲染引擎。
- 桌面壳可在团队熟悉度明确后选择：纯前端团队优先 Electron；团队能够长期维护 Rust 时可选 Tauri，以减小安装体积。
- 领域核心、规则引擎和文件格式不依赖 Electron/Tauri API，便于后续嵌入现有工作票 Web 系统。
- MVP 使用本地工程文件即可，无需先引入数据库；需要台账检索、模板中心和集中审计时，再增加本地 SQLite 或服务端存储。

这是实现建议，不是已确认技术要求。正式选型前应以目标操作系统、部署限制、团队技能和现有工作票系统技术栈进行一次短期原型验证。
...
3. 渲染模块按页面规格生成固定尺寸 SVG。
4. 输出适配器基于同一 SVG 生成 PDF、PNG 或打印内容。
...
端子是拓扑连接的最小边界。建议包含端子标识、角色、允许连接类型、电压等级和图元上的默认锚点。例如开关有进线端与出线端，三工位开关还需要能够表达不同位置下内部导通路径的变化。
...
#### 接地措施（GroundingMeasure）

工作接地线和接地刀闸应采用不同对象：

- 工作接地线包含装设端子、编号、适用文档类型和确认状态。
- 接地刀闸是正式设备，保存调度编号及拉开/合入状态。
- 现场勘察附图可显示工作接地线位置但隐藏具体编号；工作票附图必须显示并校验编号。

#### 工作范围（WorkScope）

工作范围不应只是一个矩形框。建议保存参与工作的设备和连接集合、工作任务描述、边界端子，以及需要表达的相邻/跨越带电对象。画布上的范围框只是其可视化形式。
...
| 工作角色 | 工作设备、相关停电设备、保留带电设备、范围外设备 | 决定是否必须绘出及检查规则 |
| 生命周期 | 原有、新增、拆除、改造后保留 | 支持改造前/改造后及勘察图要求 |
| 确认状态 | 自动推导、人工指定、已复核 | 保证自动结果可追溯 |

带电状态可以根据电源点、设备操作位置和连接关系辅助推导，但应满足：

1. 推导结果与人工指定冲突时必须报错，不能静默覆盖。
2. 未知状态不能直接渲染成普通停电黑色，以免产生误判。
3. 正式导出前，工作范围及边界相关设备的电气状态必须已确认。
4. 推导依据发生变化后，之前的人工复核应失效并提示重新确认。
...
- 三工位设备：运行、隔离、接地三个位置分别对应不同内部连接关系，且状态互斥。
- 熔断器：正常时可导通；熔断、拆除等状态应按业务需要单独定义，不能误用开关拉开状态。
- 母线：多个端子属于同一导电节点，但视觉上可由多个线段组成。

这样才能在拓扑图上辅助判断带电传播路径、明显断开点和接地保护范围。
...
建议优先建立常见环网柜模板，例如若干负荷开关间隔、带接地刀闸间隔、PT 间隔、变压器出线间隔。模板实例化后允许在受控范围内增减间隔，但应保留模板来源和版本。
...
- 每条从工作范围通向确认带电区域的电气路径，应存在已确认的明显断开边界；此项先作为辅助检查，由专业人员确认结果。

## 6. 第一阶段 MVP 开发计划

### 6.1 MVP 成功标准

使用软件能够独立完成一张典型 10kV 配电工作票附图，并满足以下闭环：

1. 创建或从模板生成图纸。
2. 编辑设备、状态、连接、名称、工作范围和接地措施。
3. 自动呈现带电红色、停电黑色/蓝色以及标准设备状态图元。
4. 在导出前完成核心规范校验并定位问题。
5. 保存后可无损重新打开继续编辑。
6. 导出的 PDF/PNG 与画布一致，A4/A3 彩色打印清晰。
7. 由配电专业人员使用真实案例复核，关键安全信息无遗漏、无误导。

### 6.2 MVP 设备范围

建议首批实现以下对象，足以覆盖环网柜、变压器和电缆类代表场景：

- 母线、架空线路、电缆、站内电缆头。
- 配电站内断路器、负荷开关、隔离开关、熔断式负荷开关、接地刀闸。
- 柱上断路器、柱上负荷开关、柱上隔离开关、跌落式熔断器。
- 三工位开关的运行、拉开、接地三种位置。
- 配电变压器、配电站用变、电压互感器。
- 环网柜/开关柜容器、间隔、用户站/箱式站边界。
- 工作接地线、接地点、文字标注、工作范围和保留带电提示。

电抗器、电容器、低压分支箱详细内部结构、架空变台复杂结构及勘察环境对象可以放在 MVP 后续迭代，除非试点案例表明它们是首批刚需。
...
| 里程碑 | 主要工作 | 可验证产物 |
| --- | --- | --- |
| M0：需求与图元基线（1 周） | 确认目标操作系统、票面尺寸、命名规则、首批场景；整理首批权威矢量图元；建立 8～12 份脱敏验收样例 | 已签字的 MVP 边界、图元清单、规则清单和金标准图纸 |
| M1：领域与文件内核（1～2 周） | 确定设备—端子—连接模型、状态模型、命令与撤销机制、工程文件版本 | 模型单元测试；文件保存后重新打开语义不变；旧版本迁移样例 |
| M2：基础编辑器（2 周） | 画布缩放平移、设备面板、放置/选择/移动/旋转、端子吸附、正交连线、标签编辑、属性面板、撤销重做 | 能从空白图绘制并保存简单环网柜—变压器单线图 |
| M3：安全状态与组合模板（1～2 周） | 开关状态变体、带电/停电颜色派生、接地措施、工作范围、环网柜/间隔模板、辅助带电路径推导 | 更换环网柜、更换整套变压器案例可完整表达；状态冲突能够显式报错 |
| M4：规则与输出闭环（1～2 周） | 必填项、编号、未知状态、拓扑、电压兼容、明显断开边界等检查；PDF/PNG/A4/A3 打印预览 | 错误可定位；阻断错误不能正式导出；输出与画布视觉一致 |
| M5：试点验收与加固（1～2 周） | 使用真实脱敏案例进行专业复核；修复易用性、打印和恢复问题；补齐操作日志与崩溃恢复 | 至少 4 类代表场景通过专业人员验收，连续使用无数据丢失 |

### 6.4 首批验收场景

建议选择以下四类作为金标准回归案例：

1. 更换环网柜：验证多柜、多间隔、改造前后连接和带电范围。
2. 更换整套变压器：验证高低压两侧、变压器、接地和低压不停电提示。
3. 更换箱式站高压柜：验证站房边界、多个回路、变压器及低压侧状态。
4. 电缆搭火/挑火：验证架空/电缆连接、杆号、接地点和明显断开点。

完成以上场景后，再按风险和使用频率加入土建站低压柜、架空变台及复杂临时电缆场景。

### 6.5 测试策略

- 领域模型测试：端子连接、设备状态互斥、电压兼容、删除和撤销等不变量。
- 规则测试：每条规则至少包含一个通过样例和一个失败样例，并固定规则编号。
- 文件测试：保存—打开往返一致、异常文件拒绝、版本迁移、自动恢复。
- 渲染快照测试：关键图元和典型图纸生成稳定 SVG，避免无意改变符号或颜色。
- 打印测试：A4/A3、横向/纵向、彩色与灰度提示、最小字号、线宽和页面边界。
- 专业验收：由配电人员逐项检查设备双重名称、带电范围、停电范围、断开点、接地点及编号。

### 6.6 MVP 完成定义

只有同时满足以下条件，MVP 才算完成：

- 四类首批验收场景均能从可编辑工程文件生成，不依赖粘贴整张图片。
- 核心图元、开关状态、颜色规则和导出物通过配电专业人员确认。
- 所有阻断级规则有自动测试，且不能通过普通导出入口绕过。
- 工程文件损坏、应用异常退出后有明确提示或可恢复副本，不发生静默数据丢失。
- 在目标打印机或指定 PDF 流程中完成实物验证。
- 已形成用户操作说明、图元清单、规则清单和已知限制。

## 7. 主要风险与应对

| 风险 | 影响 | 建议应对 |
| --- | --- | --- |
| 当前需求文档为空，规范未转成可验收条目 | 容易在开发后期反复修改 | M0 先形成规则清单、场景清单和金标准图纸，并由业务方确认 |
| Word 规范中的图元为图片且清晰度不一 | 图元不统一、打印失真 | 获取权威矢量源；无法获取时重绘并逐个专业审核 |
| 自动带电推导被误认为安全结论 | 可能造成错误工作票表达 | 显示推导依据和未知/冲突状态；正式导出前强制人工复核 |
| 拓扑模型与画布线条不同步 | 校验结果失真 | 连接只能通过端子命令创建；禁止以视觉相交自动认定电气连接 |
| MVP 同时覆盖高压、低压和勘察环境导致范围膨胀 | 延期且核心体验不稳定 | 先完成工作票高频对象和四类金标准场景，再扩展低压细节和环境图层 |
...
1. 目标运行环境是 Windows 桌面、浏览器，还是需要两者同时支持？是否处于内网或完全离线环境？
2. 第一阶段只交付工作票附图，还是必须同时完整交付现场勘察附图？
3. 是否需要嵌入既有工作票系统？若需要，其技术栈、身份认证和数据接口是什么？
4. 是否存在可导入的设备台账、一次接线图、GIS/PMS 数据或标准票模板？
5. 设备“双重名称”的组成、格式和校验规则是什么？不同单位是否存在差异？
6. 正式输出的纸张规格、页眉页脚、票号位置、签字栏及归档格式是什么？是否必须支持 SVG、PDF/A 或特定图片格式？
7. 黑色与蓝色是否允许用户按单位规则选择，还是正式输出只允许其中一种？
8. 首批必须覆盖哪些电压等级：仅 10kV，还是必须同时完整表达 0.4kV 低压系统？
9. 是否有权威的矢量图元库和组合柜模板？谁负责最终专业审核？
10. 带电状态是全部人工指定、根据电源点辅助推导，还是需要从外部系统导入运行方式？
11. 工作票导出前的复核、签名、审计和版本留痕是否属于本软件职责？
12. 是否有脱敏真实案例及对应的正确附图，可作为自动回归和专业验收的金标准？

## 9. 建议的下一步

在开始编码前，建议先补齐三项设计基线：

1. 将本文第 2 节的规范要求拆成带编号、严重级别和验收样例的规则清单，落入 `docs/drawing-rule.md`。
2. 由配电专业人员确认首批图元、端子、状态和组合模板，落入 `docs/equipment-model.md` 与 `docs/ring-cabinet-design.md`。
3. 明确第 8 节的产品决策，并把确认后的 MVP 范围和验收场景落入 `docs/requirements.md`。

以上三项确认后，再冻结第一版工程文件格式和领域接口，进入 MVP 实现，可显著降低后续返工风险。
